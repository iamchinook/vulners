...
<xsl:comment><xsl:value-of select="$nmap_xsl_version" /> </xsl:comment>
...
@media screen
{
  ...
  #estado {
    background-color: #bedcf2 !important;
  }  
...
<body>
  <a name="top" />
  <div id="container">
    <h1 style="width: 80%;" >Reporte DCEA - Escaneado el  <xsl:value-of select="$start" /> </h1>
    
    <ul id="menu">
      <li><a href="#scansummary">Resumen del Escaneo</a></li>
      ...
      <xsl:for-each select="host">
        ...
        <li>
          <xsl:text> | </xsl:text>
          <xsl:element name="a">
          <xsl:attribute name="id">estado</xsl:attribute>
            <xsl:attribute name="href">#host_<xsl:value-of select="translate(address/@addr, '.', '_') " style="background-color: #d7e7f2 !important;"/></xsl:attribute>
...
<xsl:template match="host">
  ...
  <xsl:choose>
    ...
    <xsl:when test="status/@state = 'up'">
      <h2 style="width: 80%;" class="up"><xsl:value-of select="address/@addr"/>
  ...
  <xsl:element name="div">
    ...
    <xsl:if test="uptime/@seconds != ''">
      <tr>
        ...
        <td><xsl:value-of select="uptime/@seconds" /> segundos  ({last} reinicio: <xsl:value-of select="uptime/@lastboot" />)
        </td>
      </tr>
    </xsl:if>
    ...
    <xsl:if test="distance/@value != ''">
      <tr>
        <td>Distancia de la red</td>
        <td><xsl:value-of select="distance/@value" /> saltos</td>
      </tr>
    </xsl:if>
    ...
    <xsl:if test="tcpsequence/@index != ''">
      <tr>
        <td>Predicción de secuencia TCP</td>
        <td>Dificultad=<xsl:value-of select="tcpsequence/@index" /> (<xsl:value-of select="tcpsequence/@difficulty" />)</td>
      </tr>
    </xsl:if>
		
    <xsl:if test="ipidsequence/@class != ''">
      <tr>
        <td>IP ID Generación de secuencia</td>
...
<xsl:template match="hostnames">
  <xsl:if test="hostname/@name != ''"><h3>Nombre de Host:</h3><ul>	<xsl:apply-templates/></ul></xsl:if>
</xsl:template>
<!-- ............................................................ -->
...
<xsl:template match="ports">
  ...
  <xsl:for-each select="extraports">
    <xsl:if test="@count > 0">
      <p>Los <xsl:value-of select="@count" /> puertos escaneados pero no mostrados a continuación están en estado: <b><xsl:value-of select="@state" /></b></p>
    </xsl:if>

    <ul>
      <xsl:for-each select="extrareasons">
        <xsl:if test="@count > 0">
          <li><p><xsl:value-of select="@count" /> puertos respondieron con: <b><xsl:value-of select="@reason" /></b></p></li>
        </xsl:if>
      </xsl:for-each>
    </ul>
  </xsl:for-each>
  ...
  <xsl:if test="count(port) > 0">
    ...
    <tr class="head">
        ...
        <td>Estado 
          <xsl:element name="a">
            <xsl:attribute name="href">javascript:togglePorts('porttable_<xsl:value-of select="$var_address"/>','cerrado');</xsl:attribute>
          </xsl:element>
          <xsl:element name="a">
            <xsl:attribute name="href">javascript:togglePorts('porttable_<xsl:value-of select="$var_address"/>','filtrado');</xsl:attribute>
          </xsl:element>
        </td>
        <td>Servicio</td>
        <td>Razon</td>
...
<xsl:template match="os">
  <h3>Detección remota del sistema operativo</h3>
		
  <xsl:if test="count(osmatch) = 0"><p>No se puede identificar el sistema operativo</p></xsl:if>

  <ul>
    <xsl:for-each select="portused">
      <li>Puerto usado: <b><xsl:value-of select="@portid" />/<xsl:value-of select="@proto" /> </b> (<b><xsl:value-of select="@state" /></b>)  </li>
    </xsl:for-each>
